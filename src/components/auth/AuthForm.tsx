    ...
    <Card className="w-full max-w-md mx-auto shadow-2xl border-0 bg-white/80 backdrop-blur-sm">
      <CardHeader className="space-y-1 text-center">
        <CardTitle className="text-2xl font-bold bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent">
          {mode === 'login' ? 'Вход в LinguaHub' : 'Регистрация'}
        </CardTitle>
        <CardDescription>
          {mode === 'login' 
            ? 'Войдите, чтобы продолжить обучение' 
            : 'Создайте аккаунт и начните изучение языков'}
        </CardDescription>
      </CardHeader>
      <CardContent>
                ...
                <RadioGroup value={role} onValueChange={(v) => setRole(v as AppRole)}>
                  <div className="flex items-center space-x-3 p-3 border rounded-lg hover:bg-muted/50 transition-colors cursor-pointer">
                    ...
                    <Label htmlFor="student" className="cursor-pointer flex-1">
                      Учиться
                      <span className="block text-xs text-muted-foreground">Найти репетитора и начать обучение</span>
                    </Label>
                  </div>
                  <div className="flex items-center space-x-3 p-3 border rounded-lg hover:bg-muted/50 transition-colors cursor-pointer">
                    <RadioGroupItem value="tutor" id="tutor" />
                    <User className="h-5 w-5 text-accent" />
                    <Label htmlFor="tutor" className="cursor-pointer flex-1">
                      Преподавать
                      <span className="block text-xs text-muted-foreground">Стать репетитором на платформе</span>
                    </Label>
                  </div>
                  <div className="flex items-center space-x-3 p-3 border rounded-lg hover:bg-muted/50 transition-colors cursor-pointer">
                    ...
                    <Label htmlFor="school" className="cursor-pointer flex-1">
                      Представлять школу
                      <span className="block text-xs text-muted-foreground">Зарегистрировать языковую школу</span>
                    </Label>
                  </div>
                </RadioGroup>
              </div>
